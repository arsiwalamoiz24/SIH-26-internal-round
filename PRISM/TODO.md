...
> **DOP stop rule (2026-08-25), amended 2026-08-26:** the DOP items below marked RESOLVED
> were settled by an 8-hypothesis investigation. Do not re-run hypotheses 1–8 as they were.
> **But the Supplementary Table 1 that rule pointed to has now arrived, and it reopens
> hypothesis 8:** PRISM ran on none of the authors' six acquisitions, so "wrong acquisition"
> is untested rather than ruled out. Three live avenues now exist — the authors' own full-pol
> acquisitions, compact-pol/hybrid-basis DOP on their compact-pol dataset, and Zhao et al.
> 2024's low-quality range-area removal. Full detail:
> `docs/SINHA_SUPPLEMENTARY_FINDINGS.md`, then `docs/DOP_GROUND_TRUTH_INVESTIGATION.md`.

- [x] **RESOLVED — no vendor DOP formula exists (2026-08-26).** The Chandrayaan-2 DFSAR User Manual (SAC/SIPG/MDPD/CH2/SAR/2020/12/23 v1.0) was read in full: it is a 7-page PDS4 archive-structure document defining product levels and file formats. It contains **no DOP formula, no CPR formula and no Stokes-parameter definitions** (zero hits for "degree of polarization", "DOP", "Stokes", "m-chi"). PRISM's general Stokes construction is therefore not contradicted by vendor documentation — there is none to contradict it. Incidentally confirmed: Level-1A SLI is single-look complex (phase-preserving, what DOP needs); Level-2A SRI is `Unsigned short int` (amplitude only).
- [x] **RESOLVED empirically (2026-08-25) — the formula is defensible, but it does not reproduce published values.** Is the DOP formula (general Stokes theory: `DOP = sqrt(S2²+S3²+S4²)/S1` from a 2×2 covariance of bias-corrected HH/VV) an acceptable stand-in? Tested directly against Sinha et al. 2026's own confirmed-ice craters: PRISM returns ~6× their reported 0.10–0.13, and 8 independently-tested hypotheses failed to close the gap. The formula is standard and the data is well-calibrated; the paper's DOP is most likely from a different processing level or polarimetric basis (their DOP-interpretation citations are hybrid/compact-pol, not quad-pol). See `docs/DOP_GROUND_TRUTH_INVESTIGATION.md`. **Consequence, corrected 2026-08-26:** CPR reproduces the paper on the same craters, which validates PRISM's radar processing and geolocation — but the paper's Supplementary Figure 6 states high CPR alone is insufficient (rough terrain reaches CPR 1.1 at DOP 0.17) and that the combined CPR-DOP criterion is required. So CPR is **not** a stand-in ice criterion, and DOP remains an open problem rather than a de-emphasised one. See `docs/SINHA_SUPPLEMENTARY_FINDINGS.md`.
